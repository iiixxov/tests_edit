...
def create_test(user_id: int, request_post) -> None:
    for name, value in request_post:
        name = name.split()[0]
        if name == 'test_name':
            _exec(f"INSERT INTO test (user_id, test_name) VALUES ('{user_id}', '{value}')")
...
def get_question_by_test_id(test_id: int) -> list:
    test = []
    for n, (test_question_id, question_text,) in enumerate(_exec(f"""
...
def user_name_by_test_id(test_id: int) -> str:
    for (name,) in _exec(f"SELECT user.name FROM user"
...
def save_trying(user_id, request_post) -> int:
    for key, value in request_post:
        if key == 'test_id':
            ...
            """)
    for (trying_id,) in _exec('SELECT trying_id FROM trying ORDER BY 1 DESC LIMIT 1'):
        return trying_id


def get_trying_answers(trying_id):
    for (question_text, answer_text, is_true) in _exec(f"""
        SELECT 
        question_text, 
        answer_text,
        CASE 
...
def get_result(trying_id):
    for (true_answers, all_answers) in _exec(F"""
        SELECT 
            ...
            COUNT(trying_answer_id)
        FROM trying_answer
        WHERE trying_id = '{trying_id}'
    """):
        return {'true': true_answers, 'all': all_answers}


def delete_test(test_id):
    _exec(f"DELETE FROM test WHERE test_id='{test_id}'")


def get_tryings_by_test_id(test_id):
    for (user_name, trying_id) in _exec(f"""
...
def get_test_id_by_trying_id(trying_id):
    for (test_id,) in _exec(f"SELECT test_id FROM trying WHERE trying_id='{trying_id}'"):
        return test_id
